...
# MAGIC %md
# MAGIC # 01_07_bronze_order_payments
# MAGIC
# MAGIC Objetivo:
# MAGIC - Ler o arquivo bruto `olist_order_payments_dataset.csv` da raw zone
# MAGIC - Preservar a estrutura original da fonte
# MAGIC - Adicionar metadados técnicos de ingestão
# MAGIC - Persistir a tabela `bronze_order_payments` em Delta
...
dbutils.widgets.text("source_file_name", "olist_order_payments_dataset.csv")
dbutils.widgets.text("target_table_name", "bronze_order_payments")
...
log_step("bronze_dataframe", "OK", "DataFrame bronze de order_payments montado com sucesso.")

# COMMAND ----------

# DBTITLE 1,Evidências pré-gravação
bronze_row_count = payments_bronze_df.count()
null_order_id_count = payments_bronze_df.filter(F.col("order_id").isNull()).count()
empty_order_id_count = payments_bronze_df.filter(F.trim(F.coalesce(F.col("order_id"), F.lit(""))) == "").count()
null_payment_type_count = payments_bronze_df.filter(F.col("payment_type").isNull()).count()
empty_payment_type_count = payments_bronze_df.filter(F.trim(F.coalesce(F.col("payment_type"), F.lit(""))) == "").count()
...
log_step("bronze_metrics", "OK", f"Null payment_type count: {null_payment_type_count}")
log_step("bronze_metrics", "OK", f"Empty payment_type count: {empty_payment_type_count}")
