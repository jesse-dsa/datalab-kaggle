...
# MAGIC %md
# MAGIC # 01_08_bronze_geolocation
# MAGIC
# MAGIC Objetivo:
# MAGIC - Ler o arquivo bruto `olist_geolocation_dataset.csv` da raw zone
# MAGIC - Preservar a estrutura original da fonte
# MAGIC - Adicionar metadados técnicos de ingestão
# MAGIC - Persistir a tabela `bronze_geolocation` em Delta
...
dbutils.widgets.text("source_file_name", "olist_geolocation_dataset.csv")
dbutils.widgets.text("target_table_name", "bronze_geolocation")
...
log_step("bronze_dataframe", "OK", "DataFrame bronze de geolocation montado com sucesso.")

# COMMAND ----------

# DBTITLE 1,Evidências pré-gravação
bronze_row_count = geolocation_bronze_df.count()
null_zip_prefix_count = geolocation_bronze_df.filter(F.col("geolocation_zip_code_prefix").isNull()).count()
empty_zip_prefix_count = geolocation_bronze_df.filter(
    F.trim(F.coalesce(F.col("geolocation_zip_code_prefix"), F.lit(""))) == ""
).count()
null_state_count = geolocation_bronze_df.filter(F.col("geolocation_state").isNull()).count()
empty_state_count = geolocation_bronze_df.filter(
    F.trim(F.coalesce(F.col("geolocation_state"), F.lit(""))) == ""
).count()

log_step("bronze_metrics", "OK", f"Row count: {bronze_row_count}")
log_step("bronze_metrics", "OK", f"Null geolocation_zip_code_prefix count: {null_zip_prefix_count}")
log_step("bronze_metrics", "OK", f"Empty geolocation_zip_code_prefix count: {empty_zip_prefix_count}")
log_step("bronze_metrics", "OK", f"Null geolocation_state count: {null_state_count}")
log_step("bronze_metrics", "OK", f"Empty geolocation_state count: {empty_state_count}")
