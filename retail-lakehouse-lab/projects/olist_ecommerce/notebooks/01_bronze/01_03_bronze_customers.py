...
# MAGIC %md
# MAGIC # 01_03_bronze_customers
# MAGIC
# MAGIC Este notebook cria a tabela Bronze `bronze_customers` a partir do arquivo
# MAGIC `olist_customers_dataset.csv` já copiado para a raw zone.
# MAGIC
# MAGIC ## O que ele faz
...
# MAGIC - Persiste a tabela `bronze_customers` em Delta
# MAGIC - Exibe evidências básicas de carga
# MAGIC
# MAGIC ## O que ele não faz
# MAGIC - Não corrige tipos analíticos nem CEP/UF
...
DEFAULTS: Dict[str, Any] = {
    ...
    "target_table_name": "bronze_customers",
...
null_customer_id_count = customers_bronze_df.filter(F.col("customer_id").isNull()).count()
empty_customer_id_count = customers_bronze_df.filter(F.trim(F.coalesce(F.col("customer_id"), F.lit(""))) == "").count()
null_customer_unique_id_count = customers_bronze_df.filter(F.col("customer_unique_id").isNull()).count()
empty_customer_state_count = customers_bronze_df.filter(F.trim(F.coalesce(F.col("customer_state"), F.lit(""))) == "").count()
empty_customer_zip_code_prefix_count = customers_bronze_df.filter(
    F.trim(F.coalesce(F.col("customer_zip_code_prefix"), F.lit(""))) == ""
).count()

evidence_rows = [
    Row(metric="bronze_row_count", value=str(bronze_row_count)),
    Row(metric="null_customer_id_count", value=str(null_customer_id_count)),
    Row(metric="empty_customer_id_count", value=str(empty_customer_id_count)),
    Row(metric="null_customer_unique_id_count", value=str(null_customer_unique_id_count)),
    Row(metric="empty_customer_state_count", value=str(empty_customer_state_count)),
    Row(metric="empty_customer_zip_code_prefix_count", value=str(empty_customer_zip_code_prefix_count)),
]
...
log_step(
    ...
    "OK",
    (
        f"Linhas={bronze_row_count}; customer_id nulos={null_customer_id_count}; "
        f"customer_id vazios={empty_customer_id_count}; "
        f"customer_unique_id nulos={null_customer_unique_id_count}; "
        f"customer_state vazios={empty_customer_state_count}; "
        f"customer_zip_code_prefix vazios={empty_customer_zip_code_prefix_count}"
    ),
)
