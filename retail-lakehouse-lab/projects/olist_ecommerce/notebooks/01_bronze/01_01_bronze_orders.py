...
# MAGIC %md
# MAGIC # 01_01_bronze_orders
# MAGIC
# MAGIC Este notebook cria a primeira tabela Bronze do projeto a partir do arquivo
# MAGIC `olist_orders_dataset.csv` já copiado para a raw zone.
# MAGIC
# MAGIC ## O que ele faz
...
# MAGIC - Persiste a tabela `bronze_orders` em Delta
# MAGIC - Exibe evidências básicas de carga
# MAGIC
# MAGIC ## O que ele não faz
# MAGIC - Não corrige datas nem tipos analíticos
...
DEFAULTS: Dict[str, Any] = {
    ...
    "target_table_name": "bronze_orders",
...
null_order_id_count = orders_bronze_df.filter(F.col("order_id").isNull()).count()
empty_order_id_count = orders_bronze_df.filter(F.trim(F.coalesce(F.col("order_id"), F.lit(""))) == "").count()
...
log_step("pre_write_checks", "OK", f"Linhas={bronze_row_count}; order_id nulos={null_order_id_count}; order_id vazios={empty_order_id_count}")
