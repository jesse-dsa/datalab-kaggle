...
# MAGIC %md
# MAGIC # 01_05_bronze_sellers
# MAGIC
# MAGIC Objetivo:
# MAGIC - Ler o arquivo bruto `olist_sellers_dataset.csv` da raw zone
# MAGIC - Preservar a estrutura original da fonte
# MAGIC - Adicionar metadados técnicos de ingestão
# MAGIC - Persistir a tabela `bronze_sellers` em Delta
...
dbutils.widgets.text("source_file_name", "olist_sellers_dataset.csv")
dbutils.widgets.text("target_table_name", "bronze_sellers")
...
log_step("bronze_dataframe", "OK", "DataFrame bronze de sellers montado com sucesso.")

# COMMAND ----------

# DBTITLE 1,Evidências pré-gravação
bronze_row_count = sellers_bronze_df.count()
null_seller_id_count = sellers_bronze_df.filter(F.col("seller_id").isNull()).count()
empty_seller_id_count = sellers_bronze_df.filter(F.trim(F.coalesce(F.col("seller_id"), F.lit(""))) == "").count()

log_step("bronze_metrics", "OK", f"Row count: {bronze_row_count}")
log_step("bronze_metrics", "OK", f"Null seller_id count: {null_seller_id_count}")
log_step("bronze_metrics", "OK", f"Empty seller_id count: {empty_seller_id_count}")
