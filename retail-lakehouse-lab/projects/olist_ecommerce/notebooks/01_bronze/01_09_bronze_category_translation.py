...
# MAGIC %md
# MAGIC # 01_09_bronze_category_translation
# MAGIC
# MAGIC Objetivo:
# MAGIC - Ler o arquivo bruto `product_category_name_translation.csv` da raw zone
# MAGIC - Preservar a estrutura original da fonte
# MAGIC - Adicionar metadados técnicos de ingestão
# MAGIC - Persistir a tabela `bronze_category_translation` em Delta
...
dbutils.widgets.text("source_file_name", "product_category_name_translation.csv")
dbutils.widgets.text("target_table_name", "bronze_category_translation")
...
log_step("bronze_dataframe", "OK", "DataFrame bronze de category_translation montado com sucesso.")

# COMMAND ----------

# DBTITLE 1,Evidências pré-gravação
bronze_row_count = category_translation_bronze_df.count()
null_category_name_count = category_translation_bronze_df.filter(F.col("product_category_name").isNull()).count()
empty_category_name_count = category_translation_bronze_df.filter(
    F.trim(F.coalesce(F.col("product_category_name"), F.lit(""))) == ""
).count()
null_category_name_english_count = category_translation_bronze_df.filter(
    F.col("product_category_name_english").isNull()
).count()
empty_category_name_english_count = category_translation_bronze_df.filter(
    F.trim(F.coalesce(F.col("product_category_name_english"), F.lit(""))) == ""
).count()

log_step("bronze_metrics", "OK", f"Row count: {bronze_row_count}")
log_step("bronze_metrics", "OK", f"Null product_category_name count: {null_category_name_count}")
log_step("bronze_metrics", "OK", f"Empty product_category_name count: {empty_category_name_count}")
log_step("bronze_metrics", "OK", f"Null product_category_name_english count: {null_category_name_english_count}")
log_step("bronze_metrics", "OK", f"Empty product_category_name_english count: {empty_category_name_english_count}")
