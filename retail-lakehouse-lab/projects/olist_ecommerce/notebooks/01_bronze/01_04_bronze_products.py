...
# MAGIC %md
# MAGIC # 01_04_bronze_products
# MAGIC
# MAGIC Este notebook cria a tabela Bronze `bronze_products` a partir do arquivo
# MAGIC `olist_products_dataset.csv` já copiado para a raw zone.
# MAGIC
# MAGIC ## O que ele faz
...
# MAGIC - Persiste a tabela `bronze_products` em Delta
# MAGIC - Exibe evidências básicas de carga
# MAGIC
# MAGIC ## O que ele não faz
# MAGIC - Não corrige tipos analíticos nem atributos de produto
...
DEFAULTS: Dict[str, Any] = {
    ...
    "target_table_name": "bronze_products",
...
null_product_id_count = products_bronze_df.filter(F.col("product_id").isNull()).count()
empty_product_id_count = products_bronze_df.filter(F.trim(F.coalesce(F.col("product_id"), F.lit(""))) == "").count()
null_product_category_name_count = products_bronze_df.filter(F.col("product_category_name").isNull()).count()
empty_product_name_lenght_count = products_bronze_df.filter(F.trim(F.coalesce(F.col("product_name_lenght"), F.lit(""))) == "").count()
empty_product_description_lenght_count = products_bronze_df.filter(
    F.trim(F.coalesce(F.col("product_description_lenght"), F.lit(""))) == ""
).count()

evidence_rows = [
    Row(metric="bronze_row_count", value=str(bronze_row_count)),
    Row(metric="null_product_id_count", value=str(null_product_id_count)),
    Row(metric="empty_product_id_count", value=str(empty_product_id_count)),
    Row(metric="null_product_category_name_count", value=str(null_product_category_name_count)),
    Row(metric="empty_product_name_lenght_count", value=str(empty_product_name_lenght_count)),
    Row(metric="empty_product_description_lenght_count", value=str(empty_product_description_lenght_count)),
]
...
log_step(
    ...
    "OK",
    (
        f"Linhas={bronze_row_count}; product_id nulos={null_product_id_count}; "
        f"product_id vazios={empty_product_id_count}; "
        f"product_category_name nulos={null_product_category_name_count}; "
        f"product_name_lenght vazios={empty_product_name_lenght_count}; "
        f"product_description_lenght vazios={empty_product_description_lenght_count}"
    ),
)
