...
# MAGIC %md
# MAGIC # 02_06_silver_sellers
# MAGIC
# MAGIC Objetivo:
# MAGIC - Ler a tabela `bronze_sellers`
# MAGIC - Padronizar campos textuais essenciais
# MAGIC - Converter colunas numéricas básicas
# MAGIC - Criar variáveis iniciais no nível do seller
# MAGIC - Persistir a tabela `silver_sellers` em Delta
...
dbutils.widgets.text("source_table_name", "bronze_sellers")
dbutils.widgets.text("target_table_name", "silver_sellers")
...
ensure_table_exists(FULL_SOURCE_TABLE_NAME, "Tabela Bronze de sellers")

# COMMAND ----------

# DBTITLE 1,Leitura da Bronze
sellers_bronze_df = spark.table(FULL_SOURCE_TABLE_NAME)
...
sellers_silver_base_df = (
    sellers_bronze_df
    .withColumn("seller_id", null_if_blank("seller_id"))
    .withColumn("seller_city", F.lower(null_if_blank("seller_city")))
    .withColumn("seller_state", F.upper(null_if_blank("seller_state")))
    .withColumn("seller_zip_code_prefix_int", sql_try_cast("seller_zip_code_prefix", "int"))
)
...
sellers_silver_df = (
    ...
    .withColumn(
        ...
        F.col("seller_zip_code_prefix_int").isNull()
    )
    .withColumn(
...
log_step("silver_dataframe", "OK", "DataFrame silver de sellers montado com sucesso.")

# COMMAND ----------

# DBTITLE 1,Evidências pré-gravação
silver_row_count = sellers_silver_df.count()
null_seller_id_count = sellers_silver_df.filter(F.col("seller_id").isNull()).count()
null_zip_code_prefix_count = sellers_silver_df.filter(F.col("seller_zip_code_prefix_int").isNull()).count()
null_seller_city_count = sellers_silver_df.filter(F.col("seller_city").isNull()).count()
null_seller_state_count = sellers_silver_df.filter(F.col("seller_state").isNull()).count()
invalid_state_code_count = sellers_silver_df.filter(F.col("is_valid_brazil_state_code") == False).count()
...
log_step("silver_metrics", "OK", f"Null seller_zip_code_prefix_int count: {null_zip_code_prefix_count}")
log_step("silver_metrics", "OK", f"Null seller_city count: {null_seller_city_count}")
log_step("silver_metrics", "OK", f"Null seller_state count: {null_seller_state_count}")
log_step("silver_metrics", "OK", f"Invalid Brazil state code count: {invalid_state_code_count}")

print("Schema da Silver:")
sellers_silver_df.printSchema()
